...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    with tf.compat.v1.variable_scope(scope):
        inp_layer = input_placeholder
        for i in range(n_layers):
            inp_layer = tf.compat.v1.layers.dense(inp_layer, size, activation=activation, kernel_initializer=tf.contrib.layers.xavier_initializer(), name='Hidden'+str(i+1))
        output_placeholder =  tf.compat.v1.layers.dense(inp_layer, output_size, activation=output_activation, kernel_initializer=tf.contrib.layers.xavier_initializer(), name='output', use_bias=False)
...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            ...
            sy_logstd = tf.compat.v1.get_variable('actionstd', shape=self.ac_dim, dtype=tf.float32, initializer=tf.compat.v1.initializers.ones)
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            # YOUR_CODE_HERE
            sy_sampled_ac = tf.compat.v1.random.categorical(sy_logits_na, 1)
        else:
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            ...
            sy_mean, sy_logstd = policy_parameters
            # YOUR_CODE_HERE
            dist = tfd.Normal(loc=[sy_mean], scale=[sy_logstd])
    ...
    def build_computation_graph(self):
        ...
        self.update_op = tf.compat.v1.train.AdamOptimizer(self.learning_rate).minimize(loss)
        ...
        if self.nn_baseline:
            ...
            self.baseline_prediction = tf.squeeze(build_mlp(
                                    ...
                                    size=self.size))
            # YOUR_CODE_HERE
            self.sy_target_n = tf.compat.v1.placeholder(shape=[None], name="target_n", dtype=tf.float32)
    ...
    def sample_trajectories(self, itr, env):
        ...
        while True:
            ...
            print('{:*^80}'.format(' Steps Completed = {:4d} '.format(timesteps_this_batch)))
    ...
    def sum_of_rewards(self, re_n):
        """
            ...
            returns:
                q_n: shape: (sum_of_path_lengths). A single vector for the estimated q values 
                    whose length is the sum of the  of the pathslengths
        ...
        if self.reward_to_go:
            for re_i in re_n:
                gamma_arr = [self.gamma**i for i in range(len(re_i))]
                reversed_gamma = np.flip(gamma_arr)
                cumsum_tau = np.flip(np.cumsum(np.flip(re_i) * reversed_gamma))
                return_tau = cumsum_tau * 1/gamma_arr
                q_n = np.hstack((q_n, return_tau))
        else:
            for re_i in re_n:
                sum_of_returns = sum(re_i * [self.gamma**i for i in range(len(re_i))])
                return_tau = [sum_of_returns] * len(re_i) #broadcast the return to len of the array
                q_n = np.hstack((q_n, return_tau))
        return q_n
    ...
    def compute_advantage(self, ob_no, q_n):
        ...
        if self.nn_baseline:
            ...
            b_n = self.sess.run(self.baseline_prediction, feed_dict={self.sy_ob_no: ob_no}) # YOUR CODE 
    ...
    def update_parameters(self, ob_no, ac_na, q_n, adv_n):
        ...
        if self.nn_baseline:
            ...
            self.baseline_update_op.run({self.sy_target_n: normalized_q_n, self.sy_ob_no: ob_no})
...
def train_PG(
    ...
    # Training Loop
    #========================================================================================#
    print('Env details', computation_graph_args)
    total_timesteps = 0
